...
PanelWindow {
    ...
    // Follow the focused monitor, so the picker opens where you are looking.
    ...
    // Exclusive, not OnDemand: the picker is opened by a keybind, so there is
    // no click to hand it the keyboard first.
    ...
    Rectangle {
        ...
        // Whole columns only: a partly-visible tile at the right edge reads as
        // a rendering glitch rather than as "there is more this way".
        readonly property int columns: Math.max(2, Math.min(6,
            Math.floor((parent.width - 60 - panel.pad * 2) / grid.cellWidth)))

        anchors.centerIn: parent
        width: grid.cellWidth * panel.columns + panel.pad * 2
        height: header.height + grid.height + footer.height + panel.pad * 2 + 26
        radius: 16
        color: Theme.base
        border.width: 1
        ...
        Item {
            ...
            // The selected filename lives here rather than under each tile: at
            // this tile width most names would elide to nothing useful.
            Text {
                ...
                text: grid.currentIndex >= 0 && grid.currentIndex < root.shown.length
            ...
            Text {
                ...
                anchors.baseline: title.baseline
                text: root.shown.length + " / " + root.wallpapers.length
            ...
            Rectangle {
                ...
                TextInput {
                    ...
                    onTextChanged: root.jumpTo(root.shown.length > 0 ? 0 : -1)
                    ...
                    Keys.onPressed: event => {
                        ...
                        // Up/Down step within a column, so in a one-row grid
                        // they are dead keys. Page with them instead.
                        case Qt.Key_Up:
                            root.jumpTo(grid.currentIndex - panel.columns); break;
                        case Qt.Key_Down:
                            root.jumpTo(grid.currentIndex + panel.columns); break;
                        case Qt.Key_Home:
                            root.jumpTo(0); break;
                        ...
                        case Qt.Key_PageDown:
                            root.jumpTo(grid.currentIndex + panel.columns); break;
                        default:
                            return; // let the field have the keystroke
                        }
                        event.accepted = true;
                    }
                }
        ...
        MouseArea {
            ...
            // Reset by show(); hover takes over on the first genuine move.
        ...
        Text {
            ...
            height: 16
            text: root.shown.length === 0
                  ? "No wallpapers match"
                  : "← →  move    ↑ ↓  page    ↵  apply    Esc  cancel    type to filter"
            color: root.shown.length === 0 ? Theme.red : Theme.overlay0
            font.family: Theme.fontFamily
