...
MenuPopup {
    ...
    // `open` is a binding here (NotificationPill drives it from
    // NotificationService.menuMonitor), so nothing may assign to it: hence
    // closeOnDismiss false and an override that goes through the service.
    closeOnDismiss: false
    onDismissed: NotificationService.closeMenu()
    function requestClose() { NotificationService.closeMenu(); }
